...
#include "ui_hardDESwidget.h"
#include <QFileDialog>
#include <QMessageBox>
#include <QFile>

hardDESwidget::hardDESwidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::hardDESwidget)
{
    ui->setupUi(this);
    ...
    unsigned char cmd[8] = {0x33, 0x05, 0x0A, 0x50, 0x55, 0x5A, 0xA0, 0xA5};
    //初始化命令
    des_cmd.append(cmd[0]);
...
hardDESwidget::~hardDESwidget()
{
    delete ui;
}

void hardDESwidget::genpushButton_click()
{
...
void hardDESwidget::keypushButton_click()
{
...
void hardDESwidget::filepushButton_click()
{
    QString filePath = QFileDialog::getOpenFileName(this, "Open File", QDir::homePath(), "Text files (*.txt)");
    if (!filePath.isEmpty())
    {
...
void hardDESwidget::encryptpushButton_click()
{
    QString folderPath = QFileDialog::getExistingDirectory(this, "Select Folder", QDir::homePath());
    if (!folderPath.isEmpty())
    {
...
void hardDESwidget::decryptpushButton_click()
{
    QString folderPath = QFileDialog::getExistingDirectory(this, "Select Folder", QDir::homePath());
    if (!folderPath.isEmpty())
    {
...
void hardDESwidget::backpushButton_click()
{
    emit sendsignal();
    this->close();
}

void hardDESwidget::serial_inf()
{
    ...
    connect(serial_port, &QSerialPort::readyRead, this, &hardDESwidget::dataRead);
}

void hardDESwidget::dataRead()
{
    readdata.append(serial_port->readAll());
}

string hardDESwidget::dataProcess()
{
...
void hardDESwidget::genKey(std::string outputname)
{
    cout << "生成密钥中......" << endl;
...
void hardDESwidget::getKey(std::string filename)
{
...
void hardDESwidget::encrypt(std::string in_hex_str)
{
    serial_port->write(des_cmd);
...
void hardDESwidget::decrypt(std::string in_hex_str)
{
    serial_port->write(des_cmd);
...
bool hardDESwidget::encrypt_file(const string filename)
{
    readdata.clear();
    ...
    for (uint64_t i = 0; i < data_len + n; i = i + 8)
    {
        ...
        temp_str = temp_str + "330550"+key_str+"555A50"+p_hex_str+"A0A5";
...
bool hardDESwidget::decrypt_file(const string filename)
{
    readdata.clear();
    ...
    for (uint64_t i = 0; i < data_len; i = i + 8)
    {
        ...
        temp_str = temp_str + "330A50"+key_str+"555A50"+c_hex_str+"A0A5";
...
std::string hardDESwidget::charToHex(unsigned char ch)
{
    std::stringstream ss;
...
QByteArray hardDESwidget::stringtoarray(string s)
{
    QByteArray array;
    unsigned char a,b,c;
    string r,t;
...
unsigned char hardDESwidget::stringtochar(string s)
{
     unsigned char ch;
     if(s=="a") ch=0x0a;
     else if(s=="b") ch=0x0b;
...
std::string hardDESwidget::intToHex(int value)
{
    stringstream stream;
...
int hardDESwidget::hexToInt(std::string hexStr)
{
