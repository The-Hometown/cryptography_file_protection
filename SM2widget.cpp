#include "SM2widget.h"
#include "ui_SM2widget.h"

#include <QFileDialog>
#include <QMessageBox>
#include <QFile>

SM2widget::SM2widget(QWidget *parent):
    QWidget(parent),
    ui(new Ui::SM2widget)
...
void SM2widget::on_genpushButton_clicked()
{
    ...
    if (!folderPath.isEmpty())
    {
        ...
        sm2.genKeys(publickeyfilePathStr, privatekeyfilePathStr);
...
void SM2widget::on_filepushButton_clicked()
{
...
void SM2widget::on_pubkeypushButton_clicked()
{
    QString filePath = QFileDialog::getOpenFileName(this, "Open File", QDir::homePath(), "Text files (*.txt)");
    if (!filePath.isEmpty())
    {
        ...
        sm2.getPublicKey(publickey_path);
        ui->textBrowser->append(("公钥文件: " + filePath));
    }
}

void SM2widget::on_prikeypushButton_clicked()
{
    QString filePath = QFileDialog::getOpenFileName(this, "Open File", QDir::homePath(), "Text files (*.txt)");
    if (!filePath.isEmpty())
    {
        ...
        sm2.getPrivateKey(privatekey_path);
        ui->textBrowser->append("私钥文件: " + filePath);
    }
}

void SM2widget::on_encryptpushButton_clicked()
{
    QString folderPath = QFileDialog::getExistingDirectory(this, "Select Folder", QDir::homePath());
    if (!folderPath.isEmpty())
    {
        ...
        sm2.encrypt_file(infile_path, outfile_path);
        ui->progressBar->setValue(100);
...
void SM2widget::on_decryptpushButton_clicked()
{
    QString folderPath = QFileDialog::getExistingDirectory(this, "Select Folder", QDir::homePath());
    if (!folderPath.isEmpty())
    {
        ...
        QFile file(fileName);
        std::string outfile_path = fileName.toStdString();
        sm2.decrypt_file(infile_path, outfile_path);
        ui->progressBar->setValue(100);
...
void SM2widget::on_backpushButton_clicked()
{
    emit sendsignal();
    this->close();
}
