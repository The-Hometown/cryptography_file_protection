...
public:
    ...
    // Encryption and decryption algorithm. Encrypt when model=0 and decrypt when model=1
    void enc_dec(uint8_t text[4][4], int model);
...
private:
    ...
    void keyExpansion(uint32_t round_keys[32]);

    // Decomposes a 32-bit integer into four 8-bit integers
    void splitIntoBytes(uint32_t num, uint8_t& byte1, uint8_t& byte2, uint8_t& byte3, uint8_t& byte4);

    // Combine four 8-bit integers into a 32-bit integer
    uint32_t joinIntegers(uint8_t data[4]);

    // tao function
    uint32_t taoFunction(uint32_t num);

    // Rotate the 32-bit integer to the left len bits
    uint32_t leftRotate(uint32_t num, uint8_t len);

    // L function
    uint32_t LFunction(uint32_t num);
