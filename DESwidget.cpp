...
void DESwidget::on_genpushButton_clicked()
{
...
void DESwidget::on_filepushButton_clicked()
{
...
void DESwidget::on_keypushButton_clicked()
{
    QString filePath = QFileDialog::getOpenFileName(this, "Open File", QDir::homePath(), "Text files (*.txt)");
    if (!filePath.isEmpty())
    {
...
void DESwidget::on_encryptpushButton_clicked()
{
    QString folderPath = QFileDialog::getExistingDirectory(this, "Select Folder", QDir::homePath());
    if (!folderPath.isEmpty())
    {
        ...
        des.encrypt_file(infile_path, outfile_path);
        ui->progressBar->setValue(100);
...
void DESwidget::on_decryptpushButton_clicked()
{
    QString folderPath = QFileDialog::getExistingDirectory(this, "Select Folder", QDir::homePath());
    if (!folderPath.isEmpty())
    {
        ...
        QFile file(fileName);
        std::string outfile_path = fileName.toStdString();
        des.decrypt_file(infile_path, outfile_path);
        ui->progressBar->setValue(100);
...
void DESwidget::on_backpushButton_clicked()
{
    emit sendsignal();
    this->close();
}
