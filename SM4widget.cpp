#include "SM4widget.h"
#include "ui_SM4widget.h"

#include <QFileDialog>
#include <QMessageBox>
#include <QFile>

SM4widget::SM4widget(QWidget *parent):
    QWidget(parent),
    ui(new Ui::SM4widget)
...
void SM4widget::on_genpushButton_clicked()
{
...
void SM4widget::on_filepushButton_clicked()
{
...
void SM4widget::on_keypushButton_clicked()
{
    QString filePath = QFileDialog::getOpenFileName(this, "Open File", QDir::homePath(), "Text files (*.txt)");
    if (!filePath.isEmpty())
    {
...
void SM4widget::on_encryptpushButton_clicked()
{
    QString folderPath = QFileDialog::getExistingDirectory(this, "Select Folder", QDir::homePath());
    if (!folderPath.isEmpty())
    {
        ...
        sm4.encrypt_file(infile_path, outfile_path);
        ui->progressBar->setValue(100);
...
void SM4widget::on_decryptpushButton_clicked()
{
    QString folderPath = QFileDialog::getExistingDirectory(this, "Select Folder", QDir::homePath());
    if (!folderPath.isEmpty())
    {
        ...
        QFile file(fileName);
        std::string outfile_path = fileName.toStdString();
        sm4.decrypt_file(infile_path, outfile_path);
        ui->progressBar->setValue(100);
...
void SM4widget::on_backpushButton_clicked()
{
    emit sendsignal();
    this->close();
}
